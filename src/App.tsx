...
export default function App() {
  ...
  useEffect(()=>{
    ...
    if(page.kind==='home')setHomeAlbumIds(ids=>stableRandomAlbumIds(albums,ids,entering,10));
  ...
  const homeTrackById=useMemo(()=>new Map(library.tracks.map(track=>[track.id,track] as const)),[library.tracks]);
  const homeContinueTracks=useMemo(()=>{
    const recent=collections.history.map(entry=>homeTrackById.get(entry.track_id)).filter((track):track is Track=>!!track);
    const candidates=player.current?[player.current,...recent]:recent,seen=new Set<number>();
    return candidates.filter(track=>{if(seen.has(track.id))return false;seen.add(track.id);return true;}).slice(0,5);
  },[collections.history,homeTrackById,player.current]);
  const homePlaylistCards=useMemo(()=>collections.playlists.slice(0,2).map(playlist=>({playlist,coverTrack:playlist.entries.map(entry=>homeTrackById.get(entry.track_id)).find((track):track is Track=>!!track)})),[collections.playlists,homeTrackById]);
    ...
    <main>
      ...
      {contentPage.kind==='home'?<div className="home-content">
        <section className="home-overview">
          <div className="page-heading"><div className="home-heading-copy"><h1>{homeCopy.title}</h1><p>Ваша музыка всегда с вами</p></div><div className="home-actions"><Tooltip content="Обновить треки из добавленных папок"><button className="subtle-button" disabled={!!progress?.running||!library.folders.length} onClick={()=>void rescanConfiguredFolders(library.folders,scan)}><RefreshCw size={17}/>Пересканировать папки</button></Tooltip><button className="subtle-button" onClick={()=>setFoldersOpen(true)}><FolderPlus size={17}/>Добавить папку</button><button className="primary-button" disabled={!library.tracks.length||player.busy} onClick={()=>void player.play(library.tracks[0],library.tracks)}><Play size={17} fill="currentColor"/>Слушать</button></div></div>
          <div className="stats-strip">
            <span className="home-stat"><span className="home-stat-icon"><Music2 size={16}/></span><span className="home-stat-copy"><strong>{library.tracks.length.toLocaleString('ru')}</strong><small>треков</small></span></span>
            <span className="home-stat"><span className="home-stat-icon"><Disc3 size={16}/></span><span className="home-stat-copy"><strong>{albums.length}</strong><small>альбомов</small></span></span>
            <span className="home-stat"><span className="home-stat-icon"><AudioLines size={16}/></span><span className="home-stat-copy"><strong>{artistCount}</strong><small>исполнителей</small></span></span>
            <span className="home-stat home-stat-hours"><span className="home-stat-icon"><Clock3 size={16}/></span><span className="home-stat-copy"><strong>{totalHours} ч</strong><small>музыки в медиатеке</small></span></span>
          </div>
        </section>
        ...
        {!!homeContinueTracks.length&&<section className="home-section home-continue-section"><div className="section-heading"><div className="home-section-title"><Clock3 size={17}/><div><h2>Продолжить слушать</h2><p>Недавно прослушанные треки</p></div></div><button className="text-button" onClick={()=>navigate({kind:'history'})}>Все <ArrowRight size={15}/></button></div><div className="home-continue-grid">{homeContinueTracks.map((track,index)=>{const current=player.status.id===track.id,progressPercent=current?Math.max(0,Math.min(100,player.status.position/Math.max(1,player.status.duration||track.duration)*100)):0;return <button className="home-continue-card" key={track.id} onClick={()=>void player.play(track,homeContinueTracks,index)} title={`Слушать ${track.title} — ${track.artist}`}><span className="home-continue-art"><Cover track={track}/><span className="home-continue-play"><Play size={15} fill="currentColor"/></span></span><span className="home-continue-copy"><strong>{track.title}</strong><small>{track.artist}</small></span>{current&&<span className="home-track-progress" role="progressbar" aria-label={`Прослушано ${Math.round(progressPercent)} процентов`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(progressPercent)}><i style={{width:`${progressPercent}%`}}/></span>}</button>;})}</div></section>}
        {!!homeRecentTracks.length&&<section className="home-section home-recent-section"><div className="section-heading"><div className="home-section-title"><AudioLines size={17}/><div><h2>Недавно добавлено</h2><p>Свежие треки из вашей медиатеки</p></div></div><button className="text-button" onClick={()=>navigate({kind:'recentlyAdded'})}>Все треки <ArrowRight size={15}/></button></div><div className="home-recent-grid">{homeRecentTracks.map((track,index)=><button className="home-recent-track" key={track.id} onClick={()=>void player.play(track,homeRecentTracks,index)} title={`Слушать ${track.title} — ${track.artist}`}><Cover track={track} size="small"/><span><strong>{track.title}</strong><small>{track.artist}</small></span><Play className="home-recent-play" size={15} fill="currentColor"/></button>)}</div></section>}
        <section className="home-section album-section"><div className="section-heading"><div className="home-section-title"><Disc3 size={17}/><div><h2>Альбомы</h2><p>Вся ваша музыкальная коллекция</p></div></div><button className="text-button" onClick={()=>navigate({kind:'albums'})}>Все альбомы <ArrowRight size={15}/></button></div>
        ...
        <section className="home-section home-playlists-section"><div className="section-heading"><div className="home-section-title"><ListMusic size={17}/><div><h2>Плейлисты</h2><p>Ваши плейлисты</p></div></div><button className="text-button" onClick={()=>navigate({kind:'playlists'})}>Все плейлисты <ArrowRight size={15}/></button></div><div className="home-playlist-grid">
          <button className="home-playlist-card" onClick={()=>navigate({kind:'liked'})}><span className="home-playlist-art home-playlist-likes"><Heart size={20}/></span><span className="home-playlist-copy"><strong>Любимые треки</strong><small>{collections.likes.length} треков</small></span><ArrowRight className="home-playlist-arrow" size={15}/></button>
          {homePlaylistCards.map(({playlist,coverTrack})=><button className="home-playlist-card" key={playlist.id} onClick={()=>navigate({kind:'playlist',id:playlist.id})}><span className="home-playlist-art">{coverTrack?<Cover track={coverTrack} size="small"/>:<ListMusic size={20}/>}</span><span className="home-playlist-copy"><strong title={playlist.name}>{playlist.name}</strong><small>{playlist.entries.length} треков</small></span><ArrowRight className="home-playlist-arrow" size={15}/></button>)}
          <button className="home-playlist-card home-playlist-create" onClick={()=>setNewPlaylist(true)}><span className="home-playlist-art"><Plus size={20}/></span><span className="home-playlist-copy"><strong>Создать плейлист</strong><small>Соберите свою атмосферу</small></span></button>
        </div></section>
